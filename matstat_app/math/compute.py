...
def resolve_start_row_index(rows_count: int, start_row_1based: int) -> int:
    ...
    if start_row_1based <= rows_count:
        ...
        return start_row_1based - 1                          # N в пределах таблицы — просто N-я строка

    m = start_row_1based % rows_count

    if m == 0:

        return rows_count - 1 # N кратно R — берём последнюю строку

    return m                  # иначе цикл: продолжаем с начала таблицы
...
def sample_row_numbers(rows_count: int, start_row_1based: int, num_rows: int = 10) -> List[int]:
    ...
    for j in range(num_rows):
        ...
        nums.append((i0 + j) % rows_count + 1)               # 10 строк подряд, при конце — снова с 1-й
...
def build_sample_array(
    ...
    for j in range(num_rows):
        ...
        for val in rows[(i0 + j) % R]:
            ...
            sample.append(float(val))                      # все числа строки подряд → n = 10×10 = 100
...
def min_max_range(x) -> Tuple[float, float, float]:
    ...
    R = x_max - x_min           # размах Rₓ = xₘₐₓ − xₘᵢₙ
...
def count_intervals(n: int) -> int:
    ...
    return int(math.floor(1.0 + 3.32 * math.log10(n)))        # сколько столбцов в таблице группировки
...
def step_h(R: float, k: int, eps_decimals: int = 2) -> float:
    ...
    h = round(h, eps_decimals)  # округляем до ε = 0,01
...
def interval_bounds(x_min: float, k: int, h: float, x_max: float) -> List[float]:
    ...
    alpha0 = x_min - h / 2.0    # первая граница чуть левее минимума

    alpha = [alpha0 + i * h for i in range(k + 1)]           # αᵢ = α₀ + i·h

    if alpha[-1] < x_max:

        alpha[-1] = x_max       # чтобы xₘₐₓ точно попал в последний интервал
    ...
    for i in range(k):
        ...
        if i == 0:
            ...
            if lo <= value <= hi:
                ...
                return i        # первый: граница входит (α₀ ≤ x ≤ α₁)

        else:

            if lo < value <= hi:

                return i        # остальные: левая граница не входит
...
def frequencies(x, alpha: List[float]) -> List[int]:
    ...
    for v in np.asarray(x).flat:
        ...
        counts[i] += 1          # считаем nᵢ — сколько точек в каждом интервале
...
def grouped_mean(x_mid_list, counts) -> float:
    ...
    for mid, ni in zip(x_mid_list, counts):
        ...
        s += mid * ni           # взвешенная сумма: nᵢ × середина интервала

    return s / n                # x̄* = (1/n) Σ nᵢ·x̄ᵢ
...
def grouped_variance(x_mid_list, counts, mean_g: float) -> float:
    ...
    for mid, ni in zip(x_mid_list, counts):
        ...
        s += ni * (mid - mean_g) ** 2                      # разброс относительно сгруппированного среднего

    return s / (n - 1)          # исправленная s*², делитель (n−1)
...
def sample_mean(x) -> float:
    ...
    return sum(data) / len(data)# точное x̄ по всем 100 значениям
...
def sample_variance(x) -> float:
    ...
    return s / (n - 1)          # точная s² для сравнения с сгруппированной
...
def analyze(x, eps_decimals: int = 2) -> AnalysisResult:
    ...
    x_min, x_max, R = min_max_range(x)                       # п. 2

    k, h, alpha = build_intervals(x, eps_decimals=eps_decimals)  # п. 3–5

    alpha_list = [float(a) for a in alpha]

    counts = frequencies(x, alpha_list)                      # п. 6–7: частоты по интервалам
    ...
    for i in range(k):
        ...
        mid = (lo + hi) / 2.0   # середина интервала для п. 8
        ...
        bins.append(
            ...
            IntervalBin(
                ...
                n_rel=ni / n,   # доля точек в интервале

                n_cum=cum,      # накопленная частота Nᵢ
    ...
    mean_g = grouped_mean(mids, counts)                     # п. 8 — по таблице

    var_g = grouped_variance(mids, counts, mean_g)

    mean_raw = sample_mean(x)   # п. 8 — точные, для отчёта
...
def _merge_small_expected(obs, exp):
    ...
    while changed and len(o) > 1:
        ...
        if e[0] < 5.0:
            ...
            changed = True    # слишком мало ожидаемых в первом столбце
            ...
            changed = True    # то же в последнем
            ...
            while i < len(o) - 1:
                ...
                if e[i] < 5.0:
                    ...
                    changed = True                         # в середине — к соседу с меньшим E
...
def chi2_normality_gof(x, alpha) -> Tuple[float, int, float, bool]:
    ...
    sigma = math.sqrt(sample_variance(data))                 # μ̂ и σ̂ из выборки
    ...
    obs = [float(c) for c in obs]                            # наблюдаемые nᵢ
    ...
    for i in range(len(obs)):
        ...
        probs.append(p)         # доля нормального закона на (αᵢ₋₁; αᵢ]

    s_prob = sum(probs)

    probs = [p / s_prob for p in probs]                      # нормируем, чтобы сумма = 1

    exp = [p * n for p in probs]# ожидаемые Eᵢ = n·pᵢ



    obs_m, exp_m = _merge_small_expected(obs, exp)

    df = len(obs_m) - 1 - 2     # r − 1 − 2 (оценили μ и σ)
    ...
    for o, e in zip(obs_m, exp_m):
        ...
        chi2 += (o - e) ** 2 / e# χ² = Σ (nᵢ−Eᵢ)²/Eᵢ

    p_value = 1.0 - stats.chi2.cdf(chi2, df)

    reject = p_value < 0.05     # p < 0,05 → нормальность отвергаем
...
def variance_test_sigma2(x, sigma0_sq: float = 1.0, alpha_level: float = 0.05):
    ...
    chi2_stat = (n - 1) * s2 / sigma0_sq                     # (n−1)·s²/σ₀², σ₀² = 1

    df = n - 1

    chi_lo = stats.chi2.ppf(alpha_level / 2, df)               # нижняя граница 2,5%

    chi_hi = stats.chi2.ppf(1 - alpha_level / 2, df)         # верхняя 97,5%

    reject = not (chi_lo <= chi2_stat <= chi_hi)               # вне интервала → H₀ отвергнута
...
def mean_test_zero(x, alpha_level: float = 0.05):
    ...
    t_stat, p_value = stats.ttest_1samp(data, 0.0)             # t = x̄/(s/√n), H₀: μ = 0

    reject = p_value < alpha_level# p < 0,05 → среднее ≠ 0
