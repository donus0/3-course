"""
Численное решение задачи Коши y' = f(t, y) методом Эйлера.
...
def plot_error_vs_time(t: np.ndarray, delta: np.ndarray, title: str, filename: str) -> None:
    plt.figure(figsize=(8, 5))
...
def task1_and2() -> None:
    ...
    for h in steps_task1:
        ...
        plot_solution_vs_exact(
            ...
            y0,
            lam,
            title=f"Задача 1: точное и численное решение (h = {h})",
            filename=f"task1_solution_h_{str(h).replace('.', '_')}.png",
        )

        plot_error_vs_time(
            t,
            delta,
            title=f"Задача 2: абсолютная ошибка по времени (h = {h})",
            filename=f"task2_error_h_{str(h).replace('.', '_')}.png",
        )
...
def task3() -> None:
    ...
    plt.title("Задача 3: ошибка в конечной точке vs шаг (логарифмические оси)")
    plt.grid(True, which="both", alpha=0.35)
    plt.gca().invert_xaxis()
    plt.tight_layout()
    plt.savefig("task3_error_vs_h.png", dpi=150)
    plt.close()
...
if __name__ == "__main__":
    ...
    task1_and2()
    task3()
    print("\nГрафики сохранены в текущую папку (PNG).")
